...
class PromptEngine {
  static const prompt_quizgen_college = 'Generate a quiz in XML format '
  ...
  static const prompt_quizgen_high_school = 'Generate a quiz in XML format '
      'that is compatible with Moodle XML import.  The quiz is to be on the subject of '
      '[subject] and should be related to [topic]. '
      'The quiz should be the same level of difficulty for high school [gradelevel] students of the '
      'English-speaking language. ';

  static const prompt_quizgen_other = 'Generate a quiz in XML format '
      'that is compatible with Moodle XML import.  The quiz is to be on the subject of '
      '[subject] and should be related to [topic]. '
  ...
  static String _FormatQuestionNumbers(AssignmentForm form) {
    ...
    if (form.multipleChoiceCount > 0) {
      ...
      prompts.add(prompt_quizgen_choice_example);
    }
    if (form.trueFalseCount > 0) {
      ...
      prompts.add(prompt_quizgen_truefalse_example);
    }
    if (form.shortAnswerCount > 0) {
      ...
      prompts.add(prompt_quizgen_shortanswer_example);
    }
    return "$ret${questionsCount.join(", ")} ${prompts.join(", ")}. ";
  }
